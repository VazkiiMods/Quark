...
public class QuarkBeaconBlockEntityRenderer {
	...
	public static void renderBeamSegment(
		...
		poseStack.pushPose();
		poseStack.translate(0.5D, 0.5, 0.5D); // Y translation changed to 0.5
		poseStack.translate(segment.offset.getX(), segment.offset.getY(), segment.offset.getZ()); // offset by the correct distance
		...
		renderPart(poseStack, bufferIn.getBuffer(RenderType.beaconBeam(textureLocation, false)), FastColor.ARGB32.color(alpha, colors), height, 0.0F, beamRadius, beamRadius, 0.0F, -beamRadius, 0.0F, 0.0F, -beamRadius, 0.0F, 1.0F, v1, v2);
		poseStack.popPose();
		v1 = (float) height * textureScale + v2;
		renderPart(poseStack, bufferIn.getBuffer(RenderType.beaconBeam(textureLocation, true)), FastColor.ARGB32.color((int)(32f*(alpha/255f)), colors), height, -glowRadius, -glowRadius, glowRadius, -glowRadius, -glowRadius, glowRadius, glowRadius, glowRadius, 0.0F, 1.0F, v1, v2);
		poseStack.popPose();
	}
	...
	private static void renderPart(PoseStack poseStack, VertexConsumer bufferIn, int colors, int height, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float u1, float u2, float v1, float v2) {
		...
		addQuad(pose, bufferIn, colors, 0, height, x1, y1, x2, y2, u1, u2, v1, v2);
		addQuad(pose, bufferIn, colors,0, height, x4, y4, x3, y3, u1, u2, v1, v2);
		addQuad(pose, bufferIn, colors, 0, height, x2, y2, x4, y4, u1, u2, v1, v2);
		addQuad(pose, bufferIn, colors, 0, height, x3, y3, x1, y1, u1, u2, v1, v2);
	}
	...
	private static void addQuad(PoseStack.Pose pose, VertexConsumer bufferIn, int colors, int yMin, int yMax, float x1, float z1, float x2, float z2, float u1, float u2, float v1, float v2) {
		addVertex(pose, bufferIn, colors, yMax, x1, z1, u2, v1);
		addVertex(pose, bufferIn, colors, yMin, x1, z1, u2, v2);
		addVertex(pose, bufferIn, colors, yMin, x2, z2, u1, v2);
		addVertex(pose, bufferIn, colors, yMax, x2, z2, u1, v1);
	}
	...
	private static void addVertex(PoseStack.Pose pose, VertexConsumer bufferIn, int colors, int y, float x, float z, float texU, float texV) {
		bufferIn.addVertex(pose, x, (float) y, z)
				.setColor(colors)
				.setUv(texU, texV)
